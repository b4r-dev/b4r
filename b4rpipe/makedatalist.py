...
def radec2str(ra, dec, inunit="rad"):
    ...
    rahabs = radeg / 15.0 * rasig
    rah = int(rahabs)
    ram = int((rahabs - rah) * 60)
    ras = (rahabs - rah - ram / 60.0) * 3600

    decsig = 1 if decdeg > 0 else -1
    decabs = decdeg * decsig
    decd = int(decabs)
    decm = int((decabs - decd) * 60)
    decs = (decabs - decd - decm / 60.0) * 3600

    return (
        "{sig}{h:02d}h{m:02d}m{s:02.2f}s".format(
            sig=" " if rasig == 1 else "-", h=rah, m=ram, s=ras
        ),
        "{sig}{h:02d}d{m:02d}m{s:02.2f}s".format(
            sig=" " if rasig == 1 else "-", h=decd, m=decm, s=decs
        ),
    )
...
def getSourceList(ffilter=None):
    ...
    for x in files:
        ...
        name = "".join(
            [
                x.decode()
                for x in f1.variables["Header.Source.SourceName"][:].tolist()
                if x != b" "
            ]
        )
...
def getNCprops(ncfile):
    ...
    try:
        ...
        srcname = "".join(
            [
                x.decode()
                for x in ncf.variables["Header.Source.SourceName"][:].tolist()
                if x != b" "
            ]
        )
        projid = "".join(
            [
                x.decode()
                for x in ncf.variables["Header.Dcs.ProjectId"][:].tolist()
                if x != b" "
            ]
        )
        observer = "".join(
            [
                x.decode()
                for x in ncf.variables["Header.Telescope.Operator"][:].tolist()
                if x != b" "
            ]
        )
        fmlofile = "".join(
            [
                x.decode()
                for x in ncf.variables["Header.B4r.FreqModulationFilename"][:].tolist()
                if x != b" "
            ]
        )
        ...
        obsgoal = "".join(
            [
                x.decode()
                for x in ncf.variables["Header.Dcs.ObsGoal"][:].tolist()
                if x != b" "
            ]
        )
        ...
        bufpos = sorted(
            np.unique(ncf.variables["Data.TelescopeBackend.BufPos"][:].tolist())
        )
...
def getXFFTSbyObsNum(obsnum, basedir):
    ...
    files = np.unique(
        [
            x[: x.find(".xfftsx")] + ".xfftsx"
            for x in glob.glob("{d}/xffts*".format(d=xfftsdir))
        ]
    )
...
def getLMTtpmByObsNum(obsnum, basedir):
    ...
    files = getFileList(
        ffilter=f"_{obsnum:06}_[0-9]{{2}}_[0-9]{{4}}.nc$", basedir=basedir
    )
    print(files)
    return files[0]
